...
/**
 * Integration tests which involve running the bcc-launcher code
...
describe('CLI tests', () => {
  const killTest = (args: string[]) => async (): Promise<void> => {
    ...
    const proc = fork(
      path.resolve(__dirname, '..', '..', 'bin', 'bcc-launcher'),
  ...
  it(
    'when the parent process is killed, bcc-node gets stopped - Sophie',
    killTest(['sophie', 'testnet', getSophieConfigDir('testnet')])
  );
});

...
import * as bccNode from '../src/bccNode';
import { ExitStatus } from '../src/bccLauncher';
...
// https://github.com/The-Blockchain-Company/bcc-node/issues/1086
const veryLongTestTimeoutMs = 70000;
const testsStopTimeout = 20;

setupExecPath();

describe('Starting bcc-wallet (and its node)', () => {
  beforeEach(before);
  afterEach(after);

  // eslint-disable-next-line jest/expect-expect
  it(
    'bcc-wallet responds to requests',
    () =>
      launcherTest(stateDir => {
        return {
          ...
          nodeConfig: {
            kind: 'sophie',
            configurationDir: getSophieConfigDir('testnet'),
            network: bccNode.networks.testnet,
          },
        };
      }),
    longTestTimeoutMs
  ...
  // eslint-disable-next-line jest/expect-expect
  it(
    'can configure the bcc-wallet to serve the API with TLS',
    async () =>
      launcherTest(stateDir => {
        return {
          stateDir,
  ...
  it(
    'handles case where bcc-node fails during initialisation',
    async () => {
      expect.assertions(5);
      let chainDir: string;
      await withMainnetConfigDir(async configurationDir => {
        const launcher = await setupTestLauncher(stateDir => {
          // bcc-node will expect this to be a directory, and exit with an error
        ...
        const expectations = new Promise<void>((done, fail) =>
          launcher.walletBackend.events.on('exit', (status: ExitStatus) => {
            try {
              ...
              // bcc-node is sometimes not exiting properly on both linux
              // and windows.
              // fixme: This assertion is disabled until the node is fixed.
              if (status.node.signal !== null) {
                loggers.test.error("Flaky test - bcc-node did not exit properly.", status.node.signal);
              }
  ...
  it(
    ...
    async () => {
      ...
      const nodeConfig =
        launcher.nodeService.getConfig() as bccNode.NodeStartService;
  ...
  it(
    'applies RTS options to bcc-node',
    async () => {
      let hp = 'bcc-node.hp';
      const launcher = await setupTestLauncher(stateDir => {

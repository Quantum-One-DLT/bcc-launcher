...
/**
 * Resolve the bcc-node configuration directory from the environment.
 * This would usually be provided by nix-shell.
 */
export function getSophieConfigDir(networkName: string): string {
  const base = process.env.BCC_NODE_CONFIGS;
  if (!base) {
    const msg =
      'BCC_NODE_CONFIGS environment variable is not set. The tests will not work.';
    console.error(msg);
    throw new Error(msg);
  }
 ...
 * Sophie mainnet.
 *
 * This is needed because files in the bcc-node configuration file
...
export async function withMainnetConfigDir<T>(
  ...
  const mainnet = getSophieConfigDir('mainnet');
      ...
      const configs = _.mapValues(
        {
          ...
          genesisCole: 'genesis-cole.json',
          genesisSophie: 'genesis-sophie.json',
          genesisAurum: 'genesis-aurum.json',

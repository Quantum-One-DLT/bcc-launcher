...
/**
 * Module for starting and managing a Bcc node and wallet backend.
 *
...
import {
  LaunchConfig,
  bccWalletStartService,
  WalletStartService,
} from './bccWallet';

import { startBccNode } from './bccNode';
...
export { LaunchConfig } from './bccWallet';
...
/**
 * Connection parameters for the `bcc-wallet` API.
 ...
 *   nodeConfig: {
 *     kind: "sophie",
 *     configurationDir: "/home/user/bcc-node/configuration/defaults/mainnet",
 *     network: {
...
export class Launcher {
  /**
   * Use this attribute to monitor and control the `bcc-wallet` process.
   */
  readonly walletService: Service;

  /**
   * Use this to access the `bcc-wallet` API server.
   */
  readonly walletBackend: WalletBackend;

  /**
   * Use this to monitor the `bcc-node` process.
   */
  ...
  private static async walletExe(
    ...
    return bccWalletStartService(baseDir, config);
  }
  ...
  private static nodeExe(
    ...
    switch (config.nodeConfig.kind) {
      case 'sophie':
        return startBccNode(baseDir, config.nodeConfig, config.networkName);
    }
  }
}

/**
 * Represents the API service of `bcc-wallet`.
 */
export class WalletBackend {
  ...
  /**
   * @deprecated
   * @return HTTP connection parameters for the `bcc-wallet` API server.
   */
  getApi(): Api {
    return this.api;
  }

...
/**
 * Module for starting and managing the bcc-wallet process.
 *
 * The main function is [[bccWalletStartService]].
 *
...
import { BccNodeConfig } from './bccNode';
...
export interface LaunchConfig {
  ...
  /**
   * TCP port to use for the `bcc-wallet` API server.
   * The default is to select any free port.
   */
  apiPort?: number;

  /**
   * IP address or hostname to bind the `bcc-wallet` API server
   ...
   * which is downloaded by bcc-wallet.
   *
  ...
  /**
   * Configuration for starting `bcc-node`.
   */
  nodeConfig: BccNodeConfig;
...
export async function bccWalletStartService(
  ...
  const base: WalletStartService = {
    ...
    extraEnv: config.stakePoolRegistryUrl
      ? {
          BCC_WALLET_STAKE_POOL_REGISTRY_URL: config.stakePoolRegistryUrl,
        }
      : undefined,
